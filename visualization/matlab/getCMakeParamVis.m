function val = getCMakeParamVis(param)
% Checks the cmake cache and extracts the stored parameter

cache_file_name = fullfile(getDrakePathVis,'pod-build','CMakeCache.txt');
if ~exist(cache_file_name,'file')
  val = [];  % fail silently
  return;
end

txt = fileread(fullfile(getDrakePathVis,'pod-build','CMakeCache.txt'));
