function [status,result] = systemWCmakeEnvVis(command)
...
if (ismac)
  command = [sprintf('export DYLD_FRAMEWORK_PATH=%s; ',getCMakeParamVis('DYLD_FRAMEWORK_PATH')), command];
  command = [sprintf('export DYLD_LIBRARY_PATH=%s; ',getCMakeParamVis('DYLD_LIBRARY_PATH')), command];
else
  command = [sprintf('export LD_LIBRARY_PATH=%s; ',getCMakeParamVis('LD_LIBRARY_PATH')), command];
end
