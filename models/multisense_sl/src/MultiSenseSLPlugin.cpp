...
#include "multisense_sl/GazeboCompat.hh"
#include "multisense_sl/MultiSenseSLPlugin.h"
...
void MultiSenseSL::Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf)
{
  ...
  if (!this->imuLink)
    gzerr << this->imuLinkName << " not found (head)\n";
...
void MultiSenseSL::LoadThread()
{
  ...
  // publish ground truth pose of robot data
  this->pubPoseQueue = this->pmq->addPub<geometry_msgs::PoseStamped>();
  this->pubPose =
    this->rosnode_->advertise<geometry_msgs::PoseStamped>(
      "/gazebo/robot_pose", 10);
...
# endif
  ...
  poseMsg.header.stamp = ros::Time(curTime.Double());
  poseMsg.header.frame_id = "base_link";

  //this->imuLink = this->atlasModel->GetLink("base_link");
  math::Pose pose = this->imuLink->GetWorldPose();
  ...
  this->pubPoseQueue->push(poseMsg, this->pubPose);
